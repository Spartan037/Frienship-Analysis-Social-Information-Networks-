...
snapchat_graph.add_node('Nupur', sec='A', gen='F')
...
snapchat_graph.add_node('Mihir', sec='B', gen='M')
...
snapchat_graph.add_node('Harshita', sec='B', gen='F')
...
snapchat_graph.add_node('Sam', sec='C', gen='M')
# snapchat_graph.add_node('Vishwajeet',sec='C',gen='M')
...
# snapchat_graph.add_node('Vedika',sec='C',gen='F')
snapchat_graph.add_node('Aishwarya', sec='C', gen='F')

# -------------------------------------------------------------------------------
snapchat_edge_list = [('Sam', 'Sumeet'), ('Sam', 'Vedant'), ('Sam', 'Apeksha'), ('Sam', 'Badri'),
                      ('Sam', 'Rahul'),
                      ('Harshita', 'Sumeet'),
                      ...
                      ('Aishwarya', 'Sam'),
                      ('Sumeet', 'Abdul'), ('Sumeet', 'Akhil'), ('Sumeet', 'Ajith'), ('Sumeet', 'Amrutha'),
                      ('Sumeet', 'Sarthak'), ('Sumeet', 'Pruthvi'),
                      ('Abdul', 'Akash'), ('Abdul', 'Sarthak'), ('Abdul', 'Darshan'), ('Abdul', 'Vedant'),
                      ('Akhil', 'Harish'), ('Akhil', 'Ajith'), ('Akhil', 'Nupur'), ('Akhil', 'Amrutha'),
                      ('Akhil', 'Darshan'), ('Akhil', 'Vedant'), ('Akhil', 'Pruthvi'),
                      ('Ajith', 'Abdul'), ('Ajith', 'Sarthak'), ('Ajith', 'Darshan'), ('Ajith', 'Vedant'),
                      ('Ajith', 'Pruthvi'), ('Ajith', 'Gaurav'), ('Ajith', 'Shreyas'),
                      ('Darshan', 'Sumeet'), ('Darshan', 'Sarthak'), ('Darshan', 'Vedant'),
                      ('Sravya', 'Harish'), ('Sravya', 'Pruthvi'), ('Sravya', 'Tejaswini'),
                      ('Vedant', 'Harish'), ('Vedant', 'Akash'), ('Vedant', 'Sarthak'), ('Vedant', 'Pruthvi'),
                      ('Pruthvi', 'Nupur'), ('Pruthvi', 'Sarthak'),
                      ('Nupur', 'Sumeet'), ('Nupur', 'Amrutha'),
                      ('Sarthak', 'Manoj'), ('Sarthak', 'Balasubramaniyan'), ('Sarthak', 'Akash'),
                      ('Sarthak', 'Sravya'), ('Sarthak', 'Shreyas'), ('Sarthak', 'Rahul'),
                      ('Mihir', 'Abdul')]
# ------------------------------------------------------------------------------------
...
for person in snapchat_graph:
    if snapchat_graph.nodes[person]['sec'] == 'A':
        nodecolorlist.append('#36688D')
    elif snapchat_graph.nodes[person]['sec'] == 'B':
...
for (u,v) in snapchat_edge_list:
    if snapchat_graph.nodes[u]['gen'] == 'M' and snapchat_graph.nodes[u]['gen'] != snapchat_graph.nodes[v]['gen']:
        B.add_node(u,gen = 'M')


for (u,v) in snapchat_edge_list:
    if snapchat_graph.nodes[v]['gen'] == 'M' and snapchat_graph.nodes[u]['gen'] != snapchat_graph.nodes[v]['gen']:
        B.add_node(v,gen = 'M')

for (u,v) in snapchat_edge_list:
    if snapchat_graph.nodes[u]['gen'] == 'F' and snapchat_graph.nodes[u]['gen'] != snapchat_graph.nodes[v]['gen']:
        B.add_node(u,gen = 'F')

for (u,v) in snapchat_edge_list:
    if snapchat_graph.nodes[v]['gen'] == 'F' and snapchat_graph.nodes[u]['gen'] != snapchat_graph.nodes[v]['gen']:
        B.add_node(v,gen = 'F')

for (u,v) in snapchat_edge_list:
    if snapchat_graph.nodes[u]['gen'] != snapchat_graph.nodes[v]['gen']:
        B.add_edge(u,v)
for person in B:
    if snapchat_graph.nodes[person]['gen'] == 'M':
...
for person in snapchat_graph:
    ...
    elif snapchat_graph.nodes[person]['gen'] == 'F':
...
for (u, v) in snapchat_edge_list:
    if snapchat_graph.nodes[u]['gen'] == 'M' and snapchat_graph.nodes[v]['gen'] == 'M':
        b2b.add_edge(u, v)
...
for person in b2b:
    ...
    elif snapchat_graph.nodes[person]['sec'] == 'C':
...
'''for (u,v) in snapchat_edge_list:
    if snapchat_graph.nodes[u]['gen'] == 'F' and snapchat_graph.nodes[u]['sec'] == 'A':
        b2b.add_node(u,gen = 'F',sec='A')
    elif snapchat_graph.nodes[u]['gen'] == 'F' and snapchat_graph.nodes[u]['sec'] == 'B':
        b2b.add_node(u,gen = 'F',sec='B')
    elif snapchat_graph.nodes[u]['gen'] == 'F' and snapchat_graph.nodes[u]['sec'] == 'C':
        b2b.add_node(u,gen = 'F',sec='C')
    else:
        b2b.add_node(u,gen = 'F', sec = 'All')

for (u,v) in snapchat_edge_list:
    if snapchat_graph.nodes[v]['gen'] == 'F' and snapchat_graph.nodes[v]['sec'] == 'A':
        b2b.add_node(v,gen = 'F',sec='A')
    elif snapchat_graph.nodes[u]['gen'] == 'F' and snapchat_graph.nodes[v]['sec'] == 'B':
        b2b.add_node(v,gen = 'F',sec='B')
    elif snapchat_graph.nodes[v]['gen'] == 'F' and snapchat_graph.nodes[v]['sec'] == 'C':
...
g2g = nx.Graph()
for (u, v) in snapchat_edge_list:
    if snapchat_graph.nodes[u]['gen'] == 'F' and snapchat_graph.nodes[v]['gen'] == 'F':
        g2g.add_edge(u, v)
...
# ---------------------------------------------------------------------------------------------
bfs_t = nx.bfs_tree(snapchat_graph, 'Mihir')
bfsnodecolorlist = []
pos = nx.shell_layout(bfs_t)
# nx.draw(G, pos, with_labels=False, arrows=True)
plt.figure(figsize=(15, 15));
for person in bfs_t:
    if snapchat_graph.has_edge(person, 'Mihir'):
...
# nx.draw_networkx(nx.dfs_tree(snapchat_graph,'Mihir'))
...
nx.draw_networkx(snapchat_graph, node_size=nodesizelistclo, node_color=nodecolorlistclo, cmap='RdYlGn',
                 edge_color='0.7')
...
pos = nx.spring_layout(snapchat_graph)
plt.figure(figsize=(20, 12.6))
nx.draw_networkx(snapchat_graph, node_size=nodesizelistbet, node_color=nodecolorlistbet, cmap='YlOrBr',
                 edge_color='0.7')
plt.savefig('snapchatbtwcent.pdf', format='pdf')
# ----------------------------------------------------------------------------------
# Edge-betweenness centrality

edgesizelistbete = [200 * nx.edge_betweenness_centrality(snapchat_graph)[(u, v)] for u, v in
                    nx.edge_betweenness_centrality(snapchat_graph).keys()]
# nodecolorlistbete = [nx.betweenness_centrality(snapchat_graph)[node] for node in snapchat_graph]
edgecolorlistbete = [nx.edge_betweenness_centrality(snapchat_graph)[(u, v)] for u, v in
                     nx.edge_betweenness_centrality(snapchat_graph).keys()]
pos = nx.spring_layout(snapchat_graph, k=1, iterations=150)
# pos = nx.spring_layout(snapchat_graph)
plt.figure(figsize=(20, 20))
nx.draw_networkx(snapchat_graph, pos, edge_color=edgecolorlistbete, width=edgesizelistbete, node_size=500,
                 node_color='#BDA589')
# nx.draw_networkx_edges(Pgirls,pos,edgelist = edgelist_grt_than_5,edge_color = 'green',width=edge_width)
# nx.draw_networkx_edge_labels(Pgirls,pos,edge_labels = labels_greater_than_5)
# plt.figure(figsize=(20,20))
...
for node in snapchat_graph:
    if snapchat_graph.nodes[node]['sec'] == 'A':
        alist.append(node)
    elif snapchat_graph.nodes[node]['sec'] == 'B':
        blist.append(node)
    elif snapchat_graph.nodes[node]['sec'] == 'C':
        clist.append(node)
    else:
        pass
# ---------------------------------------------------------------------------------
